...
const intervalIneconds = process.env.TIME || 120; 
...
const httpServer = http.createServer((req, res) => {
  if (req.url === '/') {
    ...
    const vlessURL = `vless://${UUID}@${DOMAIN}:443?encryption=none&security=tls&sni=${DOMAIN}&type=ws&host=${DOMAIN}&path=%2F?ed=2048#${NAME}-${ISP}`;
...
async function visitProjectPage() {
  try {
    ...
    if (!projectPageURL || !intervalIneconds) {
...
setInterval(visitProjectPage, intervalIneconds * 1000);
